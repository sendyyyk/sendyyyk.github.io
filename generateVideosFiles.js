...
import videosItems from './src/components/Fragments/VideosFragments/videosItems.js';
...
const videosDir = path.join(__dirname, 'src', 'components', 'Fragments/VideosFragments', 'VideosItems');
...
// Simpan nama acak yang sudah dibuat sebelumnya
const existingFiles = fs.readdirSync(videosDir).map(file => path.basename(file, '.jsx'));

// Membaca galleryItems.js yang ada
let existingVideosItems = [];
try {
    existingVideosItems = JSON.parse(fs.readFileSync(path.join(__dirname, 'src', 'components', 'Fragments', 'VideosFragments', 'videosItems.js'), 'utf-8').replace('const videosItems = ', '').replace(';', ''));
} catch (error) {
    // Jika tidak ada, kita akan menganggap tidak ada item yang ada
}

// Template untuk komponen item galeri baru
const componentTemplate = (componentName, src, videoTitle, description, poster, date) =>
...
const DisplayVideos = (props) => {
    const {componentName = "${componentName}", src = ${JSON.stringify(src)}, videoTitle = "${videoTitle}", description = "${description}", poster = ${JSON.stringify(poster)}, date = "${date}"} = props;
...
const updatedVideosItems = videosItems.map(item => {
    const randomName = item.randomName || generateRandomString(10);
    const date = item.date || getCurrentDate();
    const src = Array.isArray(item.src) ? item.src : item.src.split(',').map(video => video.trim());
    const poster = Array.isArray(item.poster) ? item.poster : item.poster.split(',').map(poster => poster.trim());
    return {
        ...item,
        randomName,
        date,
...
updatedVideosItems.forEach(item => {
    ...
    const componentFilePath = path.join(videosDir, `${componentName}.jsx`);

    if (!existingFiles.includes(componentName)) {
        fs.writeFileSync(componentFilePath, componentTemplate(componentName, src, videoTitle, description, poster, date));
        console.log(`Dibuat: ${componentFilePath}`);
    } else {
        // Jika file sudah ada, perbarui caption
        let fileContent = fs.readFileSync(componentFilePath, 'utf-8');
        if (fileContent.includes('description')) {
            const updatedContent = fileContent.replace(/description: "[^"]*"/, `description: "${description}"`);
            fs.writeFileSync(componentFilePath, updatedContent);
            console.log(`Diperbarui: ${componentFilePath}`);
        } else {
            console.log(`Tidak ada description ditemukan dalam: ${componentFilePath}`);
        }
    }
});

const videosItemsJsContent = `const videosItems = ${JSON.stringify(updatedVideosItems, null, 2)};
export default videosItems;`;

fs.writeFileSync(path.join(__dirname, 'src', 'components', 'Fragments', 'VideosFragments', 'videosItems.js'), videosItemsJsContent);
console.log('videosItems.js telah diperbarui dengan nama acak dan tanggal saat ini.');
