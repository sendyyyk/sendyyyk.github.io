...
import galleryItems from './src/components/Fragments/GalleryFragments/galleryItems.js';
...
const galleryDir = path.join(__dirname, 'src', 'components', 'Fragments/GalleryFragments', 'GalleryItems');
...
const existingFiles = fs.readdirSync(galleryDir).map(file => path.basename(file, '.jsx'));

// Membaca galleryItems.js yang ada
let existingGalleryItems = [];
try {
    existingGalleryItems = JSON.parse(fs.readFileSync(path.join(__dirname, 'src', 'components', 'Fragments', 'GalleryFragments', 'galleryItems.js'), 'utf-8').replace('const galleryItems = ', '').replace(';', ''));
} catch (error) {
    // Jika tidak ada, kita akan menganggap tidak ada item yang ada
}

// Template untuk komponen item galeri baru
const componentTemplate = (componentName, src, caption, date) =>
...
const DisplayGallery = (props) => {
    const {componentName = "${componentName}", src = ${JSON.stringify(src)}, caption = "${caption}", date = "${date}"} = props;
...
const updatedGalleryItems = galleryItems.map(item => {
    ...
    const src = Array.isArray(item.src) ? item.src : item.src.split(',').map(img => img.trim());
    return {
        ...item,
        randomName,
        date,
...
updatedGalleryItems.forEach(item => {
    ...
    const componentFilePath = path.join(galleryDir, `${componentName}.jsx`);

    if (!existingFiles.includes(componentName)) {
        fs.writeFileSync(componentFilePath, componentTemplate(componentName, src, caption, date));
        ...
        if (fileContent.includes('caption')) {
            const updatedContent = fileContent.replace(/caption: "[^"]*"/, `caption: "${caption}"`);
            fs.writeFileSync(componentFilePath, updatedContent);
            console.log(`Diperbarui: ${componentFilePath}`);
        } else {
            console.log(`Tidak ada caption ditemukan dalam: ${componentFilePath}`);
        }
    }
});

const galleryItemsJsContent = `const galleryItems = ${JSON.stringify(updatedGalleryItems, null, 2)};
export default galleryItems;`;

fs.writeFileSync(path.join(__dirname, 'src', 'components', 'Fragments', 'GalleryFragments', 'galleryItems.js'), galleryItemsJsContent);
console.log('galleryItems.js telah diperbarui dengan nama acak dan tanggal saat ini.');
